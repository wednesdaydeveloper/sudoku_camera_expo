...
## ADR-008: 透視変換は Phase 7 へ持ち越し

**状況**: Phase 5 で OCR を end-to-end で動かしたところ、サンプル画像（IMG_1509）の認識結果に **全行 +1 行ずれ** と **入力下半分のほぼ全滅** が発生。原画像が斜めに写っているため、`cropToBoundingRect` の axis-aligned 切り抜きでは盤面の格子が補正後画像で台形となり、segmentation 線とズレる。

**判断**: **Phase 5 では透視変換を実装せず、Phase 7 で対応する**。Phase 5 のスコープは「OCR を組み込んで動かす」であり、精度改善は別フェーズで扱う方針で当初から ADR-005 に明記してある。

**理由**:
- `expo-image-manipulator` には射影変換の API がなく、外部ライブラリ（`react-native-perspective-image-cropper` 等）の調査が必要
- Phase 6 で先に手動修正 UI を整備すれば、誤読のリカバリは可能
- フェーズを分けることで PR を小さく保てる

**結果**:
- Phase 5 のクローズが可能になる
- Phase 7 の DoD に「透視変換」「セル前処理」「ML Kit confidence 活用」「代替 OCR 検証」を追加
- 当面 OCR 精度は実用に耐えないが、UI で動作確認は可能

**代替案**:
- Phase 5 内で透視変換まで実装: スコープ拡大、ライブラリ選定リスク
- 自動盤面検出を先に実装: さらに大きなスコープ拡大
